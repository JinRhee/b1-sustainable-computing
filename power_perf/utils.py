...
def parse_powermetrics(path='/tmp/asitop_powermetrics', timecode="0"):
    ...
    try:
        ...
        #print(powermetrics_parse['tasks'])
        #print(powermetrics_parse['all_tasks']) # array of dictionaries
        cpu_metrics_dict = parse_cpu_metrics(powermetrics_parse)
        process_dicts = parse_process(powermetrics_parse)
        ...
        return cpu_metrics_dict, gpu_metrics_dict, thermal_pressure, bandwidth_metrics, process_dicts, timestamp
...
def run_powermetrics_process_manual(timecode, nice=10, interval=1000*100):
    #ver, *_ = platform.mac_ver()
    #major_ver = int(ver.split(".")[0])
    for tmpf in glob.glob("/tmp/asitop_powermetrics*"):
        os.remove(tmpf)
