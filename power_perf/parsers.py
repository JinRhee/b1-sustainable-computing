...
def parse_process(powermetrics_parse):
    dicts = powermetrics_parse["tasks"]
    dicts.append(powermetrics_parse["all_tasks"])
    #print(powermetrics_parse.keys())
    #process_dicts = powermetrics_parse["all_tasks"]#powermetrics_parse["tasks"].append(powermetrics_parse["all_tasks"])
    return dicts


def get_power(process_dicts, pid):
    #print(process_dicts[0]['pid'])
    #process_dict = list(filter(lambda process_dict: process_dict['pid'] == pid, process_dicts))
    process_dict = next((item for item in process_dicts if item["pid"] == pid), process_dicts[-1])
    #print(process_dict)
    #print(process_dict["name"])
    process_energy = process_dict["energy_impact"]
    #process_energy_per_s = process_dict["energy_impact_per_s"]
